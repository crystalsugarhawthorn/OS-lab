...
先仿照lab2的调试进行gdb的配置，并确保gdb可以正常工作。

## 调试过程
### 调试开启
首先开启三个终端，2个gdb调试，我们要调试exit.c文件，所以先make build-exit，然后在1号终端开启make debug，在2、3号终端开启gdb。
在2号终端找到1号终端的pid，使用指令`pgrep -f qemu-system-riscv64`找到pid为`34401`，然后`sudo gdb`，在内部使用指令`attach 34401`，之后使用指令` handle SIGPIPE nostop noprint`，防止出现SIGPIPE错误。接着就可以输入`c`，继续运行程序。然后再3号终端使用`make gdb`，首先输入`set remotetimeout unlimited`，防止出现超时错误。调试就已经初步开始了。

![alt text](image/image-1.png)

### 调试过程
在调试过程中，首先在终端3中输入`add-symbol-file obj/__user_exit.out`，添加符号表，然后打一个断点`break user/libs/syscall.c:18`，然后就可以继续输入`c`执行,在终端2中可以看到程序已经停在了断点处。
然后输入指令`x/10i $pc`，看到ecall指令在0x800104。然后，为了停到下面这段代码
```
__trapret:
    RESTORE_ALL
    ...
    sret
```
我们打断点`break kern/trap/trapentry.S:133`然后再c，就定位到了sret

![alt text](image/image-2.png)

之后就可以进行随意探索，例如下图，通过$2 = 0x0，我们可以知道，exit的返回值为0

![alt text](image/image-3.png)
![alt text](image/image-4.png)

事实上，是可以看到最左侧1号终端是在不断输出的。

### 调试结束
调试结束后，在终端2中输入`q`，结束调试，在终端3中输入`q`，结束调试。或者输入`clear`，清除断点，然后输入`c`，继续运行程序，直到程序结束。
