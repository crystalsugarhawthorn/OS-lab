...
#include <vfs.h>
#include <sysfile.h>
// LAB8 兼容性：本文件用到 MIN 计算，若未在全局头文件中定义，则本地提供一个简单宏
#ifndef MIN
...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // LAB8: 文件描述符表指针，初始为 NULL，fork/exec 初始化
        proc->filesp = NULL; // 中文注释：进程打开文件表，默认无，后续在 do_fork/copy_files 中设置
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // LAB8：复制父进程的文件表（支持 CLONE_FS 共享或复制）
